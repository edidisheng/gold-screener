...
# ── Your intro message ────────────────────────────────────────────────────
st.markdown("""
Hello! This is my investment pipeline for buying gold stocks on the ASX, mainly using EV/oz screening.
            
The **macroindicators** are mainly there for basic economic climate and are not definitive entry or exit signals, after all real yield has shown a significant change in relationship with goldprice since 2022-23 and the inverse correlation isnt always consistent.

There are two regressions, **goldlong** and **goldshort** purpose of the 1 year regression is to analyse the structural trend of gold and whether to be in it at all. The biggest deviation week section is meant to be a reference for the user to search up what happened geopolitically in the news that week to see what caused the deviation. The 90 day graph is simply to determine the best time of entry when buying gold stocks on the ASX.
...
The **Commitment of traders** gold futures is simply for a risk/exit signal when the long positions are too high
...
st.success("Macroindicators do not show strong bearish signals + Long term trend is UP and price is below trend  +  Short term price is below the 90 day trend → Strong entry point")
